...
Greedy is fast but doesn't guarantee optimal path. Best for quick approximations or when obstacles are sparse.`,
    ...
    genetic: `Genetic Evolution employs chromosome recombination, elitism, and adaptive mutations across generations.

For your grid:
- Grid complexity: ${analysis.complexity.toFixed(2)}
- Distance: ${analysis.distance} cells

Genetic algorithms excel at finding creative workarounds in highly constrained or rugged obstacle layouts where classical heuristics struggle.`,

    rl: `Deep Q-Learning (DQN) trains an autonomous neural policy using experience replay and reward reinforcement.

For your grid:
- Terrain diversity: ${analysis.mudCount + analysis.boostCount} hazard/speed zones
- Complexity: ${analysis.complexity.toFixed(2)}

DQN evaluates state transitions to develop predictive obstacle evasion and path optimization over repeated episodes.`,
  };
